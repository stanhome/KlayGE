...
public:
	void SetUp() override
	{
		...
		uavs_at_all_stage_support_ = caps.uavs_at_all_stage_support;
	...
	{
		if (!stream_output_support_ || !gs_support_ || !draw_indirect_support_ || !uavs_at_all_stage_support_)
		{
			return;
		}
...
private:
	...
	GraphicsBufferPtr DrawIndirectCopyBuffer(GraphicsBufferPtr const & vb_in, uint64_t& output_primitives)
	{
		...
		uint32_t indirect_args[4] = { 0, 1, 0, 0 };
...
private:
	...
	bool uavs_at_all_stage_support_;
	bool gs_support_;
};
